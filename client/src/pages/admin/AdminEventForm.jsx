...
export default function AdminEventForm(){
  ...
  return (
    <div className="rounded-3xl bg-white p-6 shadow-xl ring-1 ring-slate-200">
      <h2 className="mb-4 text-xl font-semibold text-slate-900">{id ? 'Edit' : 'Create'} Event</h2>
      <form onSubmit={submit} className="max-w-lg space-y-3">
        <input className="w-full rounded-lg border border-slate-300 p-2 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200" value={data.title} onChange={e=>setData({...data,title:e.target.value})} placeholder="Title" />
        <textarea className="w-full rounded-lg border border-slate-300 p-2 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200" value={data.description} onChange={e=>setData({...data,description:e.target.value})} placeholder="Description" />
        <input type="datetime-local" className="w-full rounded-lg border border-slate-300 p-2 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200" value={data.date ? new Date(data.date).toISOString().slice(0,16) : ''} onChange={e=>setData({...data,date:e.target.value})} />
        <input className="w-full rounded-lg border border-slate-300 p-2 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200" value={data.venue} onChange={e=>setData({...data,venue:e.target.value})} placeholder="Venue" />
        <input className="w-full rounded-lg border border-slate-300 p-2 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200" value={data.category} onChange={e=>setData({...data,category:e.target.value})} placeholder="Category" />
        <input type="number" className="w-full rounded-lg border border-slate-300 p-2 focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-200" value={data.capacity} onChange={e=>setData({...data,capacity:Number(e.target.value)})} />
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <button type="button" onClick={addTicketType} className="rounded-full bg-indigo-600 px-3 py-1 text-sm text-white">Add Option</button>
          </div>
          ...
          <div className="mt-4 space-y-3">
            ...
            {Array.isArray(data.ticketTypes) && data.ticketTypes.map((ticketType, index) => (
              <div key={index} className="rounded-xl bg-white p-3 shadow-sm ring-1 ring-slate-200">
        ...
        <div>
          <button className="rounded-full bg-gradient-to-r from-indigo-600 to-fuchsia-600 px-5 py-2 text-white shadow-md">Save</button>
        </div>
      </form>
    </div>
  )
